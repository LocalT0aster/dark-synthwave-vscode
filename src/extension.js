...
function activate(context) {
  ...
  let disposable = vscode.commands.registerCommand("darkSynthwave84.enableNeon", function () {
    ...
    try {
      ...
      if (!isEnabled) {
        ...
        vscode.window
          .showInformationMessage("Neon dreams is already enabled. Reload to refresh JS settings.", {
  ...
  let disable = vscode.commands.registerCommand("darkSynthwave84.disableNeon", uninstall);
...
function uninstall() {
  ...
  if (isEnabled) {
    ...
    vscode.window
      .showInformationMessage("Neon Dreams disabled. VS code must reload for this change to take effect", {
        title: "Restart editor to complete",
      })
      .then(function (msg) {
        vscode.commands.executeCommand("workbench.action.reloadWindow");
