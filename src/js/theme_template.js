(function () {
  ...
  const initNeonDreams = (disableGlow, obs) => {
    ...
    if (!disableGlow) {
      ...
      updatedThemeStyles = updatedThemeStyles.replace(
        /color: #ff7edb;/g,
        "color: #f92aad; text-shadow: 0 0 2px #100c0f, 0 0 5px #dc078e33, 0 0 10px #fff3; backface-visibility: hidden;",
      );
  ...
  const watchForBootstrap = function (mutationsList, observer) {
    for (let mutation of mutationsList) {
      if (mutation.type === "attributes") {
        ...
        const isUsingSynthwave = document.querySelector('[class*="AaronYoung-dark-synthwave-vscode-themes"]');
